                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ $order->latestPaymentVerification->bank_name ?? $order->bank_name ?? '-' }}</div>
                                <div class="text-sm text-gray-500">{{ $order->latestPaymentVerification->account_number ?? $order->account_number ?? '-' }}</div>
                                <div class="text-sm text-gray-500">{{ $order->latestPaymentVerification->account_name ?? $order->account_name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-900">Rp {{ number_format($order->latestPaymentVerification->amount ?? $order->total_price, 0, ',', '.') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($order->latestPaymentVerification?->payment_proof || $order->payment_proof)
                                <a href="{{ $order->latestPaymentVerification?->payment_proof ? Storage::url($order->latestPaymentVerification->payment_proof) : asset('foto/' . $order->payment_proof) }}" target="_blank" class="inline-flex items-center text-indigo-600 hover:text-indigo-900">
