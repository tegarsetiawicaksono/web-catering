...
<div class="max-w-6xl mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Kelola Urutan Menu</h1>
        <p class="text-gray-600 mt-2">Atur urutan tampilan menu di setiap kategori</p>
    </div>
    ...
    <div class="mb-6 bg-white p-4 rounded-lg shadow">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.ordering.menus') }}" class="px-4 py-2 {{ !$category ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }} rounded hover:bg-blue-600">
                Semua
            </a>
            @foreach ($categories as $cat)
                <a href="{{ route('admin.ordering.menus', ['category' => $cat]) }}" class="px-4 py-2 {{ $category === $cat ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }} rounded hover:bg-blue-600">
                    {{ $cat }}
                </a>
            @endforeach
        </div>
    ...
    @if ($category)
        <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded text-blue-800 text-sm">
            Menampilkan menu dari kategori: <strong>{{ $category }}</strong>
        </div>
    @endif
                ...
                @forelse ($menus as $index => $menu)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <span class="font-semibold">{{ $menu->nama }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                            <div class="flex justify-center gap-2">
                                @if ($index > 0)
                                    <form action="{{ route('admin.ordering.menus.update-order', $menu) }}" method="POST" class="inline">
                                        @csrf
