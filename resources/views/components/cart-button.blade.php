@props(['orderNotificationCount' => 0])

<!-- Cart - Only show for authenticated users -->
@auth
<div class="relative" x-data="{
  get itemCount() { 
    const count = $store.cart ? $store.cart.getCount() : 0;
    return count;
  }
}">
  <a href="{{ url('/cart') }}" class="relative inline-flex items-center justify-center w-10 h-10 hover:bg-gray-100 rounded-full group transition-colors">
