...
@php
    ...
    $normalizedCategorySlug = in_array($categorySlug, ['nasibox', 'nasi-box'], true) ? 'nasi-box' : $categorySlug;
    $isNasiBox = $normalizedCategorySlug === 'nasi-box';
    $isHampers = $normalizedCategorySlug === 'hampers';
    $categoryName = trim((string) ($category->nama ?? ''));
    $defaultImageMap = [
        'buffet' => 'foto/buffet.jpg',
        'tumpeng' => 'foto/tumpeng.jpg',
        'nasi-box' => 'foto/nasibox.jpg',
        'snack' => 'foto/snack.jpg',
        'hampers' => 'foto/buffet.jpg',
    ];
    $defaultImage = $defaultImageMap[$normalizedCategorySlug] ?? 'foto/buffet.jpg';
    ...
    $pageTitle = $isNasiBox
        ...
        : ($isHampers
            ? 'Paket Hampers'
            : 'Paket ' . ($categoryName !== '' ? $categoryName : ucfirst(str_replace('-', ' ', $normalizedCategorySlug))));
    $pageSubtitle = trim((string) ($category->deskripsi ?? '')) !== ''
        ? $category->deskripsi
        : ($isNasiBox
            ? 'Paket nasi box praktis untuk acara dan meeting Anda'
            : ($isHampers
                ? 'Pilihan hampers eksklusif untuk hadiah dan momen spesial Anda'
                : 'Pilihan menu berkualitas untuk berbagai acara Anda'));
    $emptyText = 'Belum ada menu ' . strtolower($categoryName !== '' ? $categoryName : str_replace('-', ' ', $normalizedCategorySlug)) . ' tersedia';
    $priceUnit = $isNasiBox ? 'box' : ($isHampers ? 'paket' : 'porsi');
@endphp
    ...
    <div class="relative">
        ...
        <div class="relative z-10 py-20">
            <div class="container mx-auto px-4">
                <h1 class="text-4xl md:text-5xl font-bold mb-4 text-white drop-shadow-lg">{{ $pageTitle }}</h1>
                <p class="text-lg text-white/90 max-w-2xl drop-shadow-md">{{ $pageSubtitle }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="relative z-10">
        <div class="container max-w-7xl mx-auto px-3 py-8 sm:px-4 sm:py-12 md:py-16">
            ...
            <div class="text-center py-12">
                <p class="text-gray-600 text-lg">{{ $emptyText }}</p>
            </div>
            @else
                        ...
                        <div class="w-[calc(100vw-3rem)] max-w-sm flex-shrink-0 snap-center" x-data="{ showMenu: false }">
                            <div class="menu-figure-card rounded-2xl overflow-hidden h-full">
                                <div class="relative menu-figure-image">
                                    ...
                                    @endphp
                                    <img src="{{ asset($menuImage) }}" alt="{{ $menu->nama }}" class="aspect-[4/3] w-full object-contain bg-gray-50 p-2 sm:h-64 md:h-72" onerror="this.onerror=null;this.src='{{ asset($defaultImage) }}';">
                                    @else
                                    ...
                                    <div class="absolute top-3 right-3 bg-gradient-to-r from-[#86765a] to-amber-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-sm sm:text-base font-bold shadow-lg">
                                        Rp {{ number_format($menu->harga, 0, ',', '.') }}/{{ $priceUnit }}
                                    </div>
                                ...
                                <div class="p-4 sm:p-5">
                                    ...
                                    <div class="mt-4 sm:mt-5 space-y-2">
                                        @auth
                                        <!-- Pesan Sekarang -->
                                        <a href="{{ route('checkout.show') }}?package={{ $categorySlug }}&name={{ urlencode($menu->nama) }}&price={{ $menu->harga }}&min={{ $menu->min_order }}&menu_id={{ $menu->id }}"
            ...
            <div class="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($menus as $menu)
                <div class="menu-figure-card rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                    <div class="relative menu-figure-image">
                        @if($menu->gambar)
                        @php
                            $menuImage = \Illuminate\Support\Str::startsWith($menu->gambar, ['foto/', 'http://', 'https://'])
                    ...
                    <div class="p-6 flex flex-col">
                        ...
                        <div class="space-y-3 mt-4">
                            @auth
                            <!-- Pesan Sekarang -->
                            <a href="{{ route('checkout.show') }}?package={{ $categorySlug }}&name={{ urlencode($menu->nama) }}&price={{ $menu->harga }}&min={{ $menu->min_order }}"
